---
{"dg-publish":true,"permalink":"/代码开发/Web 前端/4 前端框架/React18/32 effect 相关 hook/","dg-note-properties":{}}
---

在 React 中，用于定义副作用的因变量的 hook 有三个：
- useEffect：回调函数在 commit 阶段完成后异步执行，所以它不会阻塞视图渲染；
- useLayoutEffect：回调函数在 commit 阶段的 Layout 子阶段同步执行，一般用于执行 DOM 相关操作；
- useInsertionEffect：回调函数会在 commit 阶段的 Mutation 子阶段同步执行，与 useLayoutEffect 的区别在于执行的时候无法访问对 DOM 的引用。这个 Hook 专门为 CSS-in-JS 库插入全局的 style 元素而设计；

## 数据结构

对于这三个 effect 相关的 hook，`hook.memoizedState` 使用的是一套共同的数据结构：
...
next 字段会与当前的函数组件的其他 effect 形成环状链表，<u>连接的形式是一个单项环状链表。</u>
...
结构如下图：
![](/img/user/%E4%BB%A3%E7%A0%81%E5%BC%80%E5%8F%91/Web%20%E5%89%8D%E7%AB%AF/4%20%E5%89%8D%E7%AB%AF%E6%A1%86%E6%9E%B6/React18/_assets/2023-03-07-025835.png)
## 工作流程
...
上面的源码中，首先生成一个 hook 对象，拿到依赖，修改 Fiber 的 flag，之后将当前的 effect 推入到环状列表，`hook.memoizedState` 指向该环状列表。
...
在上面的源码中，首先从 `updateWorkInProgressHook()` 方法中拿到 hook 对象，之后从 `hook.memoizedState` 拿到存储的 effect 对象，之后会利用 `areHookInputsEqual()` 方法进行前后依赖的对比，如果依赖相同，则会在 effect 上打一个 tag，在组件渲染完以后会跳过这个 effect 的执行。

如果依赖发生了变化，那么当前的 fiberNode 会有一个 flags，回头在 commit 阶段统一执行该 effect，之后会推入新的 effect 到环状链表上面。
...
`pushEffect()` 的方法作用是生成一个 effect 对象，然后推入到当前的单向环状列表里面，相关代码如下：
...
调度阶段是 useEffect 独有的，因为 useEffect 的回调函数是在 commit 阶段完成后异步执行的，因此需要调度阶段。
...
- commitHookEffectUnmount：用于遍历 effect 链表依次执行 effect.destory 方法；
...
由于 commitHookEffectListUnmount 方法的执行世纪会先于 commitHookEffectListMount 方法执行，因此每次都会先执行 effect.destory 后才执行 effect.create。