---
{"dg-publish":true,"permalink":"/🔵 代码开发/Web 前端/2 类型系统/TypeScript/06 函数与泛型/08 逆变/","dg-note-properties":{}}
---
...
逆变也就说必须传递的是「父类型」，而不能传递「子类型」，因为在`animalToPet()`函数内调用`eat()`方法，`Pet`类型也是具有这个方法的。
