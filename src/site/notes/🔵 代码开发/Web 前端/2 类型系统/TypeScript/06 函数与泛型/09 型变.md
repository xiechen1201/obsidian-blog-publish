---
{"dg-publish":true,"permalink":"/🔵 代码开发/Web 前端/2 类型系统/TypeScript/06 函数与泛型/09 型变/","dg-note-properties":{}}
---
...
当`u1`多了一个属性`role`，结构化类型系统则认为`AdminUser`类型完全实现`User`类型，至于额外多处的`role`属性，可以认为是`AdminUser`类型继承`User`类型后添加的新属性，所以这个时候`AdminUser`类型可以被认为是`User`类型的子类型，这种方式其实就是「型变」。
...
![](/img/user/%F0%9F%94%B5%20%E4%BB%A3%E7%A0%81%E5%BC%80%E5%8F%91/Web%20%E5%89%8D%E7%AB%AF/2%20%E7%B1%BB%E5%9E%8B%E7%B3%BB%E7%BB%9F/TypeScript/06%20%E5%87%BD%E6%95%B0%E4%B8%8E%E6%B3%9B%E5%9E%8B/_assets/1733884482694-f16c0dda-e9c3-45f1-827f-a77efc1ea4ad.png)
...
也就是说每个类型的`id`属性的类型是不一样的。联合类型的父子关系的区分和基础类型是一样的，简单说：越具体，越形象化的，就是子类型。
...
TS 对于结构（对象或者类）的属性进行了协变。也就是说，如果想要保证`A`对象可以赋值给`B`对象，那么`A`对象的每个属性都必须是`B`对象对应属性的子类型。
...
只要将一个字面量赋值给变量、方法参数或者构造函数的参数，就会触发多余属性检查。
